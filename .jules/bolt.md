...
## 2026-06-25 - Add early return to structural constraints before O(N) loops
**Learning:** Checking string structure constraints (e.g., odd length for hex encodings) at the end of a validation function forces an O(N) loop to process the entire string unnecessarily for structural failures.
**Action:** Always add early returns for O(1) structural checks before executing O(N) validation loops to fail fast.
